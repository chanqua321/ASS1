...
├── Assigment1.sln
├── docs/
│   ├── PROJECT-DOCUMENTATION.md    ← tài liệu này
│   ├── Business-Roles.md           ← RBAC chi tiết
...
| File | Nội dung |
|------|----------|
| `docs/Business-Roles.md` | RBAC, endpoint, UI theo role (bản ngắn chuyên vai trò) |
| `docs/Business-Roles.doc` | Bản Word cùng chủ đề |
| `docs/Assigment1-Architecture.drawio` | Sơ đồ kiến trúc (mở bằng draw.io) |
| `Web/appsettings.json` | Cấu hình runtime thực tế |
| `Web/Program.cs` | Khởi tạo DB, auth, DI |

---

*Tài liệu được sinh để đồng bộ với codebase Assigment1. Khi thêm tính năng mới, cập nhật mục §9 (endpoint) và §11 (services) cho khớp thực tế.*
